  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto grid w-full max-w-6xl items-center gap-14 px-6 py-20 md:grid-cols-12 md:py-24 lg:gap-20">
          ...
          {profile.photoUrl && (
            ...
            >
              <img
                ...
                loading="eager"
                decoding="async"
                className="h-24 w-24 rounded-full border border-slate-200 object-cover shadow-lg shadow-slate-900/10 ring-4 ring-white dark:border-slate-700 dark:ring-slate-900"
              />
            </motion.div>
          )}
        ...
        >
          {profile.photoUrl && <HeroPortrait src={profile.photoUrl} alt={displayName} />}
        </motion.div>
      </div>
    </section>
  )
...
function HeroPortrait({ src, alt }: { src: string; alt: string }) {
  return (
    <div className="relative mx-auto aspect-square w-full max-w-[400px]">
      <div
        aria-hidden
        className="absolute inset-0 rounded-full border border-slate-300/80 dark:border-slate-700"
      />
      <div
        aria-hidden
        className="absolute inset-5 rounded-full border border-dashed border-teal-600/40 dark:border-teal-400/40"
      />
      <div
        aria-hidden
        className="absolute inset-10 rounded-full bg-gradient-teal opacity-15 blur-2xl dark:opacity-20"
      />
      <div className="absolute inset-8 overflow-hidden rounded-full border border-white bg-slate-100 shadow-2xl shadow-slate-900/15 ring-1 ring-slate-200 dark:border-slate-900 dark:bg-slate-800 dark:ring-slate-700">
        <img
          src={src}
          alt={alt}
          loading="eager"
      ...
      <span
        aria-hidden
        className="absolute right-[1.2%] top-1/2 h-2.5 w-2.5 -translate-y-1/2 rounded-full bg-teal-600 ring-4 ring-slate-50 dark:bg-teal-400 dark:ring-slate-950"
      />
    </div>
  )
}
